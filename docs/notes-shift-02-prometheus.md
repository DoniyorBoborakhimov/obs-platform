...
sensitive = true скрывает значение в ВЫВОДЕ команд, но НЕ в файле состояния.
tfstate всегда содержит все значения открытым текстом.

Следствия:
1. tfstate — секретный файл. Никогда не коммитить.
2. Локальный state не годится для команды: нет блокировок, конфликты при
   параллельном apply. Нужен удалённый бэкенд (S3+DynamoDB, TF Cloud, GitLab).
3. Удалённый бэкенд должен быть зашифрован и версионирован.

Секреты НЕ кладут в values-файлы: они попадают и в git, и в вывод plan
(который печатается в логи CI и комментарии PR), и в state.

Правильные способы передачи:
- переменная окружения TF_VAR_<имя> (локально, в CI — из секретов пайплайна)
- набор set_sensitive для Helm
- внешний менеджер секретов: Vault, AWS Secrets Manager, External Secrets Operator
- SOPS для шифрования файлов, которые всё же нужны в git
